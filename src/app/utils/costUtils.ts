import { Enchantable, Enchant, EnchantableType, EnchantableWithCost } from "../models/enchanted.book";
...
export let logText = "";
export let cache: Map<string, EnchantableWithCost> = new Map<string, EnchantableWithCost>();

export function findBestCombination(enchantElements) {
    if (enchantElements.length > 1) {
        ...
        let result: EnchantableWithCost = findBestCombinationRecursive(enchantElements.map(el => ({ cost: 0, totalCost: 0, maxCost: 0, enchant: el })));
        ...
        console.log('                        total: ', totalInvalidPermutations + totalValidPermutations);
...
export function findBestCombinationRecursive(enchantElementsClone: EnchantableWithCost[]): EnchantableWithCost {
        ...
        return (inverseOrderCost < normalOrderCost) ?
            {
                ...
                enchant: forge(enchantElementsClone[1].enchant, enchantElementsClone[0].enchant),
            ...
            :
            {
                ...
                enchant: forge(enchantElementsClone[0].enchant, enchantElementsClone[1].enchant),
                children: [enchantElementsClone[0], enchantElementsClone[1]]
            };
    }

    const permutations = getPermutations(enchantElementsClone, enchantElementsClone.length);
    ...
    permutations.map((el, index) => {
        ...
        if (bestCostSplited.totalCost < 10000000) {
            ...
            resultsBrief.set(bestCostSplited.totalCost + '(' + bestCostSplited.maxCost + ')', !resultsBrief.get(bestCostSplited.totalCost + '(' + bestCostSplited.maxCost + ')') ? 1 : resultsBrief.get(bestCostSplited.totalCost + '(' + bestCostSplited.maxCost + ')') + 1)
        } else {
    ...
    const costs: number[] = [];
    resultsBrief.forEach((el, key) => costs.push(el));
    const gcdValue = gcd(costs);
    resultsBrief.forEach((el, key) => resultsBrief.set(key, el));
    resultsBrief = new Map([...resultsBrief.entries()].sort());
    console.log('Best of all permutations: ', printEnchantableCost(bestWayToForge));
...
export function gcd_two_numbers(x: number, y: number) {
    let x2 = Math.abs(x);
    let y2 = Math.abs(y);
    while (y2) {
        let t = y2;
...
export function findBestWayToForge(list: EnchantableWithCost[]): EnchantableWithCost {
    ...
    const newLastPairEnchant: EnchantableWithCost = {
        ...
        enchant: forge(list[list.length - 2].enchant, list[list.length - 1].enchant),
...
export function findBestCostSplittingBy(initialGroupSize: number, enchants: EnchantableWithCost[]): EnchantableWithCost {
    const lists: EnchantableWithCost[][][] = getPermutations(enchants, initialGroupSize);
    const result: EnchantableWithCost = { cost: 9199999999, totalCost: 99999999999999, maxCost: 981822222222, enchant: null };
    lists.map(pair => {
        let enchant1 = findBestCombinationRecursive(pair[0]);
        let enchant2 = findBestCombinationRecursive(pair[1]);
...
export function getPermutations(enchants: EnchantableWithCost[], initialGroupSize: number): EnchantableWithCost[][][] {
    ...
    if (initialGroupSize == 1) {
        ...
        const lists: EnchantableWithCost[][][] = getPermutations(enchants, initialGroupSize - 1);
        lists.map(list => {
            let particionAFaltaDe1: EnchantableWithCost[] = list[0];
            let onePartitions: EnchantableWithCost[][][] = getPermutations(list[1], 1);
...
export function printEnchantableWithCost(enchant: EnchantableWithCost, tabs = 0, trace = true) {
    ...
    if (!enchant.enchant && trace) {
        logText += (!str ? '' : '|') + str + enchant.enchant + '[totalCost:' + enchant.totalCost + '] - [maxCost:' + enchant.maxCost + '] - [cost:' + enchant.cost + '] - ' + printEnchantable(enchant.enchant) + '\n';
        console.log((!str ? '' : '|') + str + enchant.enchant + '[totalCost:' + enchant.totalCost + '] - [maxCost:' + enchant.maxCost + '] - [cost:' + enchant.cost + '] - ' + printEnchantable(enchant.enchant))
    } else {
        logText += (!str ? '' : '|') + str + enchant.enchant.type + '[totalCost:' + enchant.totalCost + '] - [maxCost:' + enchant.maxCost + '] - [cost:' + enchant.cost + '] - [penalty:' + enchant.enchant.penalty + '] - ' + printEnchantable(enchant.enchant) + '\n';
        console.log((!str ? '' : '|') + str + enchant.enchant.type + '[totalCost:' + enchant.totalCost + '] - [maxCost:' + enchant.maxCost + '] - [cost:' + enchant.cost + '] - [penalty:' + enchant.enchant.penalty + '] - ' + printEnchantable(enchant.enchant))
    }
    printEnchantableWithCostList(enchant.children, tabs + 1, trace);
}

export function printEnchantableWithCostList(children: [EnchantableWithCost, EnchantableWithCost], tabs: number, trace = true) {
    if (!children || !trace) return;
    children.map(el => printEnchantableWithCost(el, tabs))
}

export function print3array(result: EnchantableWithCost[][][], trace = true): any {
    if (!trace) return;
    result.map(pair => console.log(printEnchantableList(pair[0].map(el => el.enchant)) + ' con ' + printEnchantableList(pair[1].map(el => el.enchant))));
}

export function printEnchantableList(enchants: Enchantable[]): string {
    return '[' + enchants.map(el => /*'{' + el.type + '}' + */printEnchantable(el)).join(', ') + ']';
}
...
export function forge(target: Enchantable, sacrifice: Enchantable): Enchantable {
